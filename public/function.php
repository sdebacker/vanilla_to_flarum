...
{
    global $dbFlarum, $dbVanilla, $dbVanillaPrefix, $dbFlarumPrefix;
    ...
    $text = preg_replace_callback(
        ...
        function ($matches) use ($dbVanilla, $dbVanillaPrefix) {
            $id = $matches[1];
            if (isset($matches[2])) {
                $id = $matches[2];
            }
            $query = RunPreparedQuery($dbVanilla, [':id' => $id], "SELECT forum_name FROM ${dbVanillaPrefix}forums WHERE id = :id");
